...
# 创建数据加载器
train_iter = DataLoader(train_data, batch_size=16, shuffle=True, num_workers=4)
val_iter = DataLoader(val_data, batch_size=16, shuffle=False, num_workers=4)
test_iter = DataLoader(test_data, batch_size=16, shuffle=False, num_workers=4)
...
def train(train_iter, val_iter, test_iter):
    ...
    optimizer = torch.optim.Adam(model.fc.parameters(), lr=0.0001, weight_decay=0.005) # 使用权重衰减

    # 创建学习率调度器
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=2, verbose=True) # 使用ReduceLROnPlateau
    ...
    torch.save(model.state_dict(), 'Convresults/model3.pth')
