...
model_path = './results/My_model/mobilenetv2-25-e-2.ckpt'
...
# 获取所有图片的路径
image_paths = glob.glob('./datasets/5fbdf571c06d3433df85ac65-momodel/garbage_26x100/val/**/*.jpg')

# 随机选择9张图片
selected_image_paths = random.sample(image_paths, 9)
...
for i, image_path in enumerate(selected_image_paths):
    ...
    plt.imshow(image)
    # 设置标题
    plt.title('Predicted: {}'.format(predict(image)))
    # 关闭坐标轴
    plt.axis('off')
