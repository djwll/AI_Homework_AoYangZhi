    ...
    for i in range(total_steps):
        if i < warmup_steps:
            ...
            if decay_type == 'cosine':
                ...
                exponential_decay = (0.1)**(total_steps/decay_steps)
...
# MobileNetV2Backbone 是一个神经网络模型或者模型的定义，用于提取图像特征
backbone = MobileNetV2Backbone()
# 加载预训练的权重或检查点文件到模型中。config.pretrained_ckpt 是预训练权重的路径，net=backbone 表示将加载的权重应用到 backbone 这个模型上
load_checkpoint(config.pretrained_ckpt, net=backbone)
# 使用 backbone 模型提取特征并保存到指定的路径
...
class MobileNetV2Head(nn.Cell):
    ...
    def __init__(self, input_channel=1280, hw=7, num_classes=1000, reduction='mean', activation="none", dropout_rate=0.5):
